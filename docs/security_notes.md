...
Optional relaxed modes when sync-on-write is disabled:
- `AIONBD_WAL_SYNC_EVERY_N_WRITES=<N>`
- `AIONBD_WAL_SYNC_INTERVAL_SECONDS=<seconds>` (for example `10`)

These reduce, but do not remove, loss risk. With interval-based sync, an abrupt crash can
lose acknowledged writes since the last successful fsync window.
