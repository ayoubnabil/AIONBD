...
2. 5xx ratio: `rate(aionbd_http_responses_total{status="5xx"}[5m]) / clamp_min(rate(aionbd_http_requests_total[5m]), 1)`
...
3. If intentional, consider bounded risk settings:
   `AIONBD_WAL_SYNC_EVERY_N_WRITES` and/or `AIONBD_WAL_SYNC_INTERVAL_SECONDS`.
4. Log the durability tradeoff decision in incident/change notes.
...
- `AIONBD_WAL_SYNC_INTERVAL_SECONDS` adds time-based WAL fsync cadence, but acknowledged writes since the last fsync can still be lost on abrupt crash.
