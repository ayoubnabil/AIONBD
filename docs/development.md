...
AIONBD_BENCH_SCENARIO=collection_churn cargo run --release -p aionbd-bench
...
Open-source benchmark wrapper (ANN-Benchmarks dataset + local AIONBD vs `hnswlib`, with optional Qdrant when Docker daemon is available):
```bash
python3 scripts/run_ann_open_bench_wrapper.py
```

Reproducibility mode (repeat runs + aggregated mean/median/min/max in JSON/Markdown):
```bash
python3 scripts/run_ann_open_bench_wrapper.py --engines aionbd,qdrant --repeat 5
```
...
- `AIONBD_WAL_SYNC_INTERVAL_SECONDS` (default: `0`; when `AIONBD_WAL_SYNC_ON_WRITE=false`, force fsync at least once every N seconds)
...
- `AIONBD_WAL_SYNC_INTERVAL_SECONDS=10` can still lose up to around 10s of acknowledged writes.
...
Compare current reports against versioned dry-run baselines:
```bash
...
5. Avoid oversized files: split files before they exceed 300 lines.
   Legacy exceptions are tracked in `scripts/check_file_sizes.allowlist` and
   should be reduced over time, not expanded casually.
