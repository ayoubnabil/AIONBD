...
def default_profiles() -> list[dict[str, object]]:
    base = {
        ...
        "dimension": 256,
        "timeout_seconds": 5.0,
        "strict_max_error_rate": float(os.environ.get("AIONBD_SOAK_MAX_ERROR_RATE", "0.05")),
        "min_throughput_ops_per_second": float(os.environ.get("AIONBD_SOAK_MIN_THROUGHPUT_OPS_PER_SECOND", "0")),
    ...
    return [{**base, "name": "read_heavy", "write_ratio": 0.10}, {**base, "name": "mixed", "write_ratio": 0.30}]
...
def validate_profile(profile: dict[str, object]) -> None:
    ...
    checks = [
        ...
        (0.0 <= float(profile["write_ratio"]) <= 1.0, "write_ratio must be in [0.0, 1.0]"),
...
def run_profile(profile: dict[str, object], base_url: str, collection_prefix: str, reports_dir: Path, recreate_collections: bool, dry_run: bool) -> dict[str, object]:
    if dry_run:
    ...
    command = [
        ...
        str(SOAK_SCRIPT),
        "--base-url", base_url,
        "--collection", f"{collection_prefix}_{profile_name}",
        "--dimension", str(profile["dimension"]),
        "--duration-seconds", str(profile["duration_seconds"]),
        "--workers", str(profile["workers"]),
        "--write-ratio", str(profile["write_ratio"]),
        "--point-space", str(profile["point_space"]),
        "--metric", str(profile["metric"]),
        "--search-mode", str(profile["search_mode"]),
        "--search-limit", str(profile["search_limit"]),
        "--timeout-seconds", str(profile["timeout_seconds"]),
        "--strict-max-error-rate", str(profile["strict_max_error_rate"]),
    ...
    for field in REQUIRED_REPORT_FIELDS:
        if field not in payload:
            raise RuntimeError(f"soak profile '{profile_name}' missing report field: {field}")
    return payload
...
def evaluate(profile: dict[str, object], row: dict[str, object]) -> list[str]:
    ...
    if float(row["error_rate"]) > float(profile["strict_max_error_rate"]):
        failures.append(f"profile={name} error_rate={row['error_rate']} > max={profile['strict_max_error_rate']}")
    if float(row["throughput_ops_per_second"]) < float(profile["min_throughput_ops_per_second"]):
        failures.append(f"profile={name} throughput_ops_per_second={row['throughput_ops_per_second']} < min={profile['min_throughput_ops_per_second']}")
    if float(row["latency_us_p95"]) > float(profile["max_latency_us_p95"]):
        failures.append(f"profile={name} latency_us_p95={row['latency_us_p95']} > max={profile['max_latency_us_p95']}")
    if float(row["latency_us_p99"]) > float(profile["max_latency_us_p99"]):
        failures.append(f"profile={name} latency_us_p99={row['latency_us_p99']} > max={profile['max_latency_us_p99']}")
    return failures
...
def markdown_report(generated_at: str, rows: list[dict[str, object]]) -> str:
    ...
    for row in rows:
        lines.append("| {profile} | {duration:.3f} | {workers} | {write_ratio:.3f} | {throughput:.3f} | {error_rate:.6f} | {p50} | {p95} | {p99} |".format(
            profile=row["profile"],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run AIONBD soak profiles and report results")
    ...
    parser.add_argument("--report-path", default=os.environ.get("AIONBD_SOAK_REPORT_PATH", "bench/reports/soak_pipeline_report.md"))
    parser.add_argument("--report-json-path", default=os.environ.get("AIONBD_SOAK_REPORT_JSON_PATH", "bench/reports/soak_pipeline_report.json"))
    parser.add_argument("--profile-reports-dir", default=os.environ.get("AIONBD_SOAK_PROFILE_REPORTS_DIR", "bench/reports/soak_profiles"))
    return parser.parse_args()

def main() -> int:
    args = parse_args()
    if args.profiles_file:
        os.environ["AIONBD_SOAK_PROFILES_JSON"] = Path(args.profiles_file).read_text(encoding="utf-8")
    ...
    failures: list[str] = []
    reports_dir = Path(args.profile_reports_dir)
    for profile in profiles:
        row = run_profile(
            profile,
