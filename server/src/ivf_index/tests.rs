...
fn candidate_ids_reduce_search_space() {
    ...
    let index = IvfIndex::build(&collection).expect("index should build");
    let candidate_ids = index.candidate_ids_with_target_recall(&[1_005.0, 0.0], 10, None);
...
fn higher_recall_target_expands_candidate_pool() {
    ...
    let index = IvfIndex::build(&collection).expect("index should build");
    let low = index.candidate_ids_with_target_recall(&[3.0, 1.0], 20, Some(0.2));
    let high = index.candidate_ids_with_target_recall(&[3.0, 1.0], 20, Some(1.0));
    assert!(!low.is_empty());
