...
pub(crate) fn remove_l2_index_entry(state: &AppState, collection_name: &str) {
    let _ = state.l2_indexes.remove(collection_name);
}
    ...
    tokio::spawn(async move {
        ...
        let build_result = tokio::task::spawn_blocking(move || {
            ...
            let (dimension, len, mutation_version, snapshot_points) = {
                ...
                let snapshot_points: Vec<(usize, Vec<f32>)> = (0..collection.slot_count())
                    .filter_map(|slot| {
                        collection
                            .point_at_slot(slot)
                            .map(|(_, values)| (slot, values.to_vec()))
                    })
                    .collect();
                (
        ...
        match build_result {
            Ok(Ok(Some(index))) => {
                state
                    .l2_indexes
                    .insert(collection_name.clone(), Arc::new(index));
                let _ = state
                    .metrics
