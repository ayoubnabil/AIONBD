...
async fn metrics_reports_collection_and_point_counts() {
    ...
    assert_eq!(payload["http_responses_5xx_total"], 0);
    assert!(
    ...
    assert_eq!(payload["persistence_wal_sync_interval_seconds"], 0);
    assert_eq!(payload["persistence_wal_group_commit_max_batch"], 16);
    assert_eq!(payload["persistence_wal_group_commit_flush_delay_ms"], 0);
...
async fn metrics_reflect_runtime_flags_and_write_counter() {
    ...
    assert_eq!(payload["http_responses_5xx_total"], 2);
    assert!(
        payload["http_request_duration_us_total"]
            .as_u64()
            .unwrap_or(0)
