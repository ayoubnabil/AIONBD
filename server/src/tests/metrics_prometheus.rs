...
async fn metrics_prometheus_reports_text_metrics() {
    ...
    assert!(payload.contains("aionbd_l2_index_build_max_in_flight 2"));
    assert!(payload.contains("aionbd_l2_index_warmup_on_boot 1"));
    assert!(payload.contains("aionbd_http_requests_total"));
    assert!(payload.contains("aionbd_http_requests_in_flight 1"));
    assert!(payload.contains("aionbd_http_responses_total{status=\"2xx\"} 3"));
    assert!(payload.contains("aionbd_http_responses_total{status=\"4xx\"} 0"));
    assert!(payload.contains("aionbd_http_responses_total{status=\"5xx\"} 0"));
    ...
    assert!(payload.contains("aionbd_persistence_wal_sync_interval_seconds 0"));
...
async fn metrics_prometheus_reflects_runtime_flags() {
    ...
    assert!(payload.contains("aionbd_http_responses_total{status=\"2xx\"} 6"));
    assert!(payload.contains("aionbd_http_responses_total{status=\"4xx\"} 2"));
    assert!(payload.contains("aionbd_http_responses_total{status=\"5xx\"} 1"));
    assert!(payload.contains("aionbd_http_request_duration_us_total "));
