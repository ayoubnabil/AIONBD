...
async fn unknown_collection_writes_do_not_grow_lock_map() {
    ...
    let before = state.collection_write_locks.len();
    ...
    let after = state.collection_write_locks.len();
...
async fn remove_collection_write_lock_keeps_shared_lock_and_removes_when_idle() {
    ...
    let shared_lock = {
        ...
        state.collection_write_locks.insert(name.to_string(), lock);
        cloned
    };
    ...
    assert!(state.collection_write_locks.contains_key(name));
...
async fn stale_write_lock_entry_is_pruned_after_missing_collection_write() {
    ...
    state.collection_write_locks.insert(
        "ghost".to_string(),
    ...
    let upsert_resp = app
        ...
        .expect("response expected");
    assert_eq!(upsert_resp.status(), StatusCode::NOT_FOUND);
    assert!(!state.collection_write_locks.contains_key("ghost"));
