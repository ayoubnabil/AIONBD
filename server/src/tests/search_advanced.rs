...
#[tokio::test]
async fn search_can_skip_payload_in_response() {
    let app = build_app(test_state());
    ...
    let create_req = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(
            json!({"name": "search_skip_payload", "dimension": 2}).to_string(),
        ))
        .expect("request must build");
    let create_resp = app
        .clone()
    ...
    let upsert_req = Request::builder()
        .method("PUT")
        .uri("/collections/search_skip_payload/points/1")
        .header("content-type", "application/json")
        .body(Body::from(
            json!({"values": [1.0, 0.0], "payload": {"category":"keep"}}).to_string(),
        ))
        .expect("request must build");
    let upsert_resp = app
        .clone()
    ...
    let search_req = Request::builder()
        .method("POST")
        .uri("/collections/search_skip_payload/search/topk")
        .header("content-type", "application/json")
        .body(Body::from(
            json!({
                "query": [1.0, 0.0],
    ...
    assert_eq!(payload["hits"][0]["id"], 1);
    assert!(payload["hits"][0].get("payload").is_none());
}
...
async fn search_mode_ivf_supports_non_l2_metric_with_l2_candidate_prefilter() {
    ...
    state
        .l2_indexes
        .insert("search_ivf_non_l2".to_string(), std::sync::Arc::new(index));
...
async fn search_mode_ivf_without_target_recall_avoids_quality_full_scan() {
    ...
    state
        .l2_indexes
        .insert("search_ivf_fast".to_string(), std::sync::Arc::new(index));
    let app = build_app(state);
