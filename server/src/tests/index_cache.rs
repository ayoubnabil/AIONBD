...
fn seed_cached_l2_index(state: &AppState, name: &str) {
    ...
    state
        .l2_indexes
        .insert(name.to_string(), std::sync::Arc::new(index));
}
...
async fn upsert_existing_point_invalidates_cached_l2_index() {
    ...
    assert!(state.l2_indexes.contains_key("cache_upsert"));
...
async fn upsert_new_point_invalidates_cached_l2_index() {
    ...
    assert!(!state.l2_indexes.contains_key("cache_upsert_new"));
}
...
async fn delete_point_invalidates_cached_l2_index() {
    ...
    assert!(state.l2_indexes.contains_key("cache_delete_point"));
...
async fn delete_collection_invalidates_cached_l2_index() {
    ...
    assert!(state.l2_indexes.contains_key("cache_delete"));
...
fn invalidating_cache_does_not_cancel_in_flight_build_marker() {
    ...
    assert!(!state.l2_indexes.contains_key("cache_keep_building"));
    assert!(state
        .l2_index_building
        .read()
        .expect("l2 index building lock should be available")
...
async fn wal_append_failure_keeps_cached_l2_index() {
    ...
    assert!(state.l2_indexes.contains_key("cache_wal_fail"));
