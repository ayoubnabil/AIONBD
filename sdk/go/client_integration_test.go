...
func TestClientIntegration(t *testing.T) {
	...
	created, err := client.CreateCollection(ctx, collectionName, 4, true)
	if err != nil {
		t.Fatalf("create collection failed: %v", err)
	}
	if created.Name != collectionName {
...
func startServer(t *testing.T) (string, func()) {
	...
	command := exec.CommandContext(ctx, "cargo", "run", "-q", "-p", "aionbd-server")
