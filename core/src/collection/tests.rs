...
#[test]
fn reuses_freed_slots_without_leaking_removed_points() {
    let mut collection = new_collection(true);
    collection
        .upsert_point(1, vec![1.0, 1.0, 1.0])
        .expect("must succeed");
    collection
        .upsert_point(2, vec![2.0, 2.0, 2.0])
        .expect("must succeed");

    let removed = collection.remove_point(1).expect("point must exist");
    assert_eq!(removed, vec![1.0, 1.0, 1.0]);

    collection
        .upsert_point(3, vec![3.0, 3.0, 3.0])
        .expect("must succeed");

    assert_eq!(collection.len(), 2);
    assert!(collection.get_point(1).is_none());
    assert_eq!(collection.get_point(2), Some(&[2.0, 2.0, 2.0][..]));
    assert_eq!(collection.get_point(3), Some(&[3.0, 3.0, 3.0][..]));
    assert_eq!(collection.point_ids(), vec![2, 3]);

    let mut unordered_ids: Vec<PointId> = collection
        .iter_points_unordered()
        .map(|(id, _values)| id)
        .collect();
    unordered_ids.sort_unstable();
    assert_eq!(unordered_ids, vec![2, 3]);
}

#[test]
fn compacts_internal_slots_after_heavy_deletes() {
    let mut collection = new_collection(true);
    for id in 0_u64..600 {
        collection
            .upsert_point(id, vec![id as f32, 1.0, 2.0])
            .expect("must succeed");
    }
    assert_eq!(collection.len(), 600);
    assert_eq!(collection.slot_count(), 600);

    for id in 0_u64..556 {
        let removed = collection.remove_point(id).expect("point must exist");
        assert_eq!(removed[0], id as f32);
    }

    assert_eq!(collection.len(), 44);
    assert_eq!(collection.slot_count(), 44);
    assert_eq!(collection.point_ids().len(), 44);
    assert_eq!(collection.get_point(555), None);
    assert_eq!(collection.get_point(556), Some(&[556.0, 1.0, 2.0][..]));
}

#[test]
fn delete_point_fast_path_removes_without_materializing_values() {
    let mut collection = new_collection(true);
    collection
        .upsert_point(42, vec![4.0, 2.0, 0.0])
        .expect("must succeed");

    assert!(collection.delete_point(42));
    assert!(!collection.delete_point(42));
    assert!(collection.get_point(42).is_none());
    assert!(collection.is_empty());
}
