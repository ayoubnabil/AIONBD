...
pub fn append_wal_record_with_sync(
    ...
    append_wal_record_with_sync_info(wal_path, record, sync_on_write).map(|_| ())
}
...
pub fn append_wal_records_with_sync(
    ...
    append_wal_records_with_sync_info(wal_path, records, sync_on_write).map(|_| ())
}

pub fn append_wal_record_with_sync_info(
    wal_path: &Path,
    record: &WalRecord,
    sync_on_write: bool,
...
pub fn append_wal_records_with_sync_info(
    wal_path: &Path,
    records: &[WalRecord],
    sync_on_write: bool,
) -> Result<WalAppendInfo, PersistenceError> {
